...
describe('MessageList', () => {
  it('user 消息渲染文本，无头像', () => {
    render(<MessageList onPicError={() => {}} onPicTap={() => {}} items={[base]} />);
    expect(screen.getByText('你好')).toBeTruthy();
    expect(document.querySelector('.human-corner-tag')).toBeNull();
  });

  it('assistant 文本消息渲染头像+文本', () => {
    render(<MessageList onPicError={() => {}} onPicTap={() => {}} items={[{ ...base, id: 'm2', role: 'assistant', content: { content: '回复' } }]} />);
    expect(screen.getByText('回复')).toBeTruthy();
  });

  it('boss_reply 渲染「人工」金标 + human 头像徽章', () => {
    render(<MessageList onPicError={() => {}} onPicTap={() => {}} items={[{
  ...
  it('image 加载失败（picBroken）→ 占位卡片', () => {
    ...
    render(<MessageList onPicError={() => {}} onPicTap={() => {}} items={[broken]} />);
